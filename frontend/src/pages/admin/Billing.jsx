...
export default function Billing() {
  ...
  const addToCart = (product) => {
    setCart(prev => {
      ...
      if (ex) {
        if (ex.qty >= product.quantity) return toast.error('Stock limit reached') || prev
        return prev.map(i => i.productId === product._id ? { ...i, qty: i.qty + 1 } : i)
      }
      return [...prev, { productId: product._id, name: product.name, mrp: product.mrp, discountPercent: product.discountPercent, finalPrice: product.finalPrice, maxQty: product.quantity, qty: 1 }]
    })
  }
  ...
  const updateQty = (id, val) => {
    ...
    if (n > item.maxQty) return toast.error('Exceeds available stock')
  ...
  const discount = cart.reduce((s, i) => s + (i.mrp - i.finalPrice) * i.qty, 0)
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>Select products and generate a bill for your customer.</p>
      </div>
      ...
      {bill ? (
        ...
        <div className="pos-grid">
          ...
          <div>
            ...
            <div className="card">
              <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 14 }}>Select Products</h3>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 420, overflowY: 'auto' }}>
                {filtered.length === 0 ? (
                  <p style={{ color: 'var(--text-muted)', fontSize: 13, textAlign: 'center', padding: '20px 0' }}>No in-stock products found.</p>
                ) : filtered.map(p => (
          ...
          <div className="card" style={{ position: 'sticky', top: 80 }}>
            ...
            {cart.length === 0 ? (
              <p style={{ color: 'var(--text-muted)', fontSize: 13, textAlign: 'center', padding: '32px 0' }}>Add products from the left panel.</p>
            ) : (
              <>
